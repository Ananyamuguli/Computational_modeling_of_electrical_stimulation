...
def glutamate_analysis(run):
    ...
    for g in extrasyn_glu:
        print(g)
        _, x_, pts = ffe(Istim1=2, extrasyn_glu=g, forward_syn_weight=10)
...
def atp_dependency(run):
    ...
    for atp in atp_levels1:
        print(atp)
        _, x_, pts = ffe(Istim1=2, atp=atp, forward_syn_weight=10)
        fr_pre, fr_post = firing_rate_cal_func(pts)
        freq_firing_pre.append(fr_pre)
        freq_firing_post.append(fr_post)
        del _, x_, pts
...
def mito_dependency(run):
    #file_path = '/storage/hpc/work/aaz/am209/analysis_data/motif1/'
    mito_levels = np.arange(0, 180, 5)
    freq_firing_pre = []
    freq_firing_post = []
    for mito in mito_levels:
        print(mito)
        _, x_, pts = ffe(Istim1=2, del_phi_mito=mito, forward_syn_weight=10)
        fr_pre, fr_post = firing_rate_cal_func(pts)
        freq_firing_pre.append(fr_pre)
        freq_firing_post.append(fr_post)
        del _, x_, pts
...
def Ko_dependency(run):
    ...
    for Ko in Ko_levels:
        print(Ko)
        _, x_, pts = ffe(Istim1=2, Ko_rest = Ko, forward_syn_weight=10)
        fr_pre, fr_post = firing_rate_cal_func(pts)
        freq_firing_pre.append(fr_pre)
        freq_firing_post.append(fr_post)
        del _, x_, pts
...
def synweight_dependency(run):
    syn_weight = np.arange(0,40,1) #Dimensionless
    freq_firing_pre = []
    freq_firing_post = []
    for syn in syn_weight:
        print(syn)
        _,x_, pts = ffe(Istim1=2, forward_syn_weight=syn)
        fr_pre, fr_post = firing_rate_cal_func(pts)
        freq_firing_pre.append(fr_pre)
        freq_firing_post.append(fr_post)
        del _, x_, pts
